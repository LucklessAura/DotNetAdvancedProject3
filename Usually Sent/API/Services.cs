...
;                query.ToList().ForEach(el =>
        ...
        public byte[] GetImageBytes(int fileId)
        {
            var query = from p in databaseContext.Files where p.FileId == fileId select p;
        ...
        public List<string> GetDistinctProprietyNames()
        {
            List<string> proprieties = new List<string>();
            var query = from p in databaseContext.AditionalFileProprieties select p.ProprietyName;
            proprieties = query.Distinct().ToList();
            return proprieties;
        }
    }
}
